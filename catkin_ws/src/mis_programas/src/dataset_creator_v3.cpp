...
void random_set(unsigned int *n_train_total, unsigned int *n_valid_total, unsigned int *n_test_total, std::string path_depth_maps, std::string path_labels_files, std::string path_train, std::string path_valid, std::string path_test, std::string path_train_TXT, std::string path_valid_TXT, std::string path_test_TXT, std::string path_info_valid_TXT, std::string path_info_test_TXT){
    ...
    std::vector<std::string> ficheros_NO_train_dm;
    std::vector<std::string> ficheros_NO_train_label;
    std::vector<std::string> ficheros_random_NO_train_dm;
    std::vector<std::string> ficheros_random_NO_train_label;
    ...
    //std::cout << "\n";
    for (int i = 0; i < (0.7*(numero_ficheros)); i++){
    ...
    for (int i = 0; i < ficheros_dm.size(); i++){ //Compruebo que ficheros estan ya en train y los que no los separo para valid y test
        int check = 0;
        for(int j = 0; j < ficheros_random_dm.size(); j++){
        ...
        if(check == 0){
            ficheros_NO_train_dm.push_back(ficheros_dm[i]);
            ficheros_NO_train_label.push_back((path_labels_files + "/" + ficheros_dm_names[i]+ ".txt"));
        }
    ...
    int numero_no_train = ficheros_NO_train_dm.size();

    for (int i = 0; i < (((2.0)/(3.0))*(numero_no_train)); i++){ //20% van a valid
        int p = rand() % (numero_no_train-1) + 0;
        //std::cout << "La p es: "<< p << "\n"; 
        if(ficheros_random_NO_train_dm.empty()){
            ficheros_random_NO_train_dm.push_back(ficheros_NO_train_dm[p]);
            ficheros_random_NO_train_label.push_back((ficheros_NO_train_label[p]));
        }
        else{
            int check = 0;
            for(int j = 0; j < ficheros_random_NO_train_dm.size(); j++){
                if (ficheros_random_NO_train_dm[j]==ficheros_NO_train_dm[p]){
                    check = 1;
                }
            }
            if(check == 0){
    ...
    std::sort(ficheros_random_NO_train_dm.begin(),ficheros_random_NO_train_dm.end(),ordenar_nombres_string);
    std::sort(ficheros_random_NO_train_label.begin(),ficheros_random_NO_train_label.end(),ordenar_nombres_string);


    for(int i = 0; i < ficheros_random_NO_train_dm.size(); i++){
        
        boost::filesystem::copy_file(ficheros_random_NO_train_dm[i],(path_valid + "/" + std::to_string(*n_valid_total) + ".png"), boost::filesystem::copy_option::overwrite_if_exists);
        boost::filesystem::copy_file(ficheros_random_NO_train_label[i],(path_valid + "/" + std::to_string(*n_valid_total) + ".txt"), boost::filesystem::copy_option::overwrite_if_exists);
        file_valid.open(path_valid_TXT, std::fstream::out | std::fstream::app);
        file_valid << "build/darknet/x64/data/obj/valid/" << *n_valid_total << ".png\n";
        file_valid.close();
        file_info.open(path_info_valid_TXT, std::fstream::out | std::fstream::app);
        file_info << "El fichero:  valid/" << *n_valid_total << ".png coresponde a:\n";
        file_info << ficheros_random_NO_train_dm[i] << "\n\n";
        file_info.close();
        *n_valid_total = *n_valid_total + 1;
    }


    for (int i = 0; i < ficheros_NO_train_dm.size(); i++){
        int check = 0;
        for(int j = 0; j < ficheros_random_NO_train_dm.size(); j++){
            if(ficheros_random_NO_train_dm[j]==ficheros_NO_train_dm[i]){
                check = 1;
            }
        }
        if(check == 0){
            ficheros_test_dm.push_back(ficheros_NO_train_dm[i]);
            ficheros_test_label.push_back((ficheros_NO_train_label[i]));
        }
    ...
    for(int i = 0; i < ficheros_test_dm.size(); i++){
        ...
        file_test.close();
        file_info.open(path_info_test_TXT, std::fstream::out | std::fstream::app);
...
int main(){
    ...
    std::string path_valid_txt = path_image_set + "/valid.txt";
    std::string path_test_txt = path_image_set + "/test.txt";
    std::string path_train = path_image_set + "/train";
    std::string path_valid = path_image_set + "/valid";
    std::string path_test = path_image_set + "/test";
    
    std::string path_info_valid = path_image_set + "/info_valid";
    std::string path_info_test = path_image_set + "/info_test";
    ...
    if(!(boost::filesystem::exists(path_valid))){
        std::cerr << "AVISO: \n";
        std::cerr << "NO existe el directorio " << path_valid << "\n";
        std::cerr << "Se va a crear. \n";
        if(boost::filesystem::create_directory(path_valid)){
            std::cout << path_valid << " creada correctamente \n";
        }
        else{
            std::cout << "ERROR: no se pudo crear " << path_valid << std::endl;
            return -1;
        }        
    }
    else{
        if(!(boost::filesystem::is_empty(path_valid))){
            std::cerr << "Atención NO está vacío el directorio " << path_valid <<"\n";
            std::cerr << "Para evitar sobreesritura accidental se detiene este programa \n";
            std::cerr << "Para vaciar el fichero en la carpeta de la que cuelga ejecutar este comando: \n";
            std::cerr << "rm -r test/*\n";
            return -1;
    ...
    if(!(boost::filesystem::exists(path_info_valid))){
        std::cerr << "AVISO: \n";
        std::cerr << "NO existe el directorio " << path_info_valid << "\n";
        std::cerr << "Se va a crear. \n";
        if(boost::filesystem::create_directory(path_info_valid)){
            std::cout << path_info_valid << " creada correctamente \n";
        }
        else{
            std::cout << "ERROR: no se pudo crear " << path_info_valid << std::endl;
            return -1;
        }        
    }

    if(!(boost::filesystem::exists(path_info_test))){
        std::cerr << "AVISO: \n";
        std::cerr << "NO existe el directorio " << path_info_test << "\n";
        std::cerr << "Se va a crear. \n";
        if(boost::filesystem::create_directory(path_info_test)){
            std::cout << path_info_test << " creada correctamente \n";
        }
        else{
            std::cout << "ERROR: no se pudo crear " << path_info_test << std::endl;
            return -1;
        }        
    }
    ...
    file_info.open(path_info_valid +"/experimento_1.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 1\n";
    file_info << "Correspondencia ficheros valid con los ficheros originales: \n\n";
    file_info.close();
    file_info.open(path_info_test +"/experimento_1.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 1\n";
    file_info << "Correspondencia ficheros test con los ficheros originales: \n\n";
    file_info.close();

    random_set(&n_total_train, &n_total_valid,&n_total_test, path_depth_map_1,path_label_1,path_train,path_valid, path_test,path_train_txt, path_valid_txt, path_test_txt,(path_info_valid +"/experimento_1.txt"),(path_info_test +"/experimento_1.txt"));
    unsigned int train_1 = n_total_train;
    unsigned int valid_1 = n_total_valid;
    unsigned int test_1 = n_total_test;

    file_info.open(path_info_valid +"/experimento_2.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 2\n";
    file_info << "Correspondencia ficheros valid con los ficheros originales: \n\n";
    file_info.close();
    file_info.open(path_info_test +"/experimento_2.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 2\n";
    file_info << "Correspondencia ficheros test con los ficheros originales: \n\n";
    file_info.close();

    random_set(&n_total_train, &n_total_valid,&n_total_test, path_depth_map_2,path_label_2,path_train,path_valid, path_test,path_train_txt, path_valid_txt, path_test_txt,(path_info_valid +"/experimento_2.txt"),(path_info_test +"/experimento_2.txt"));
    unsigned int train_2 = n_total_train - train_1;
    unsigned int valid_2 = n_total_valid - valid_1;
    unsigned int test_2 = n_total_test - test_1;

    file_info.open(path_info_valid +"/experimento_3.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 3\n";
    file_info << "Correspondencia ficheros valid con los ficheros originales: \n\n";
    file_info.close();
    file_info.open(path_info_test +"/experimento_3.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 3\n";
    file_info << "Correspondencia ficheros test con los ficheros originales: \n\n";
    file_info.close();

    random_set(&n_total_train, &n_total_valid,&n_total_test, path_depth_map_3,path_label_3,path_train,path_valid, path_test,path_train_txt, path_valid_txt, path_test_txt,(path_info_valid +"/experimento_3.txt"),(path_info_test +"/experimento_3.txt"));
    unsigned int train_3 = n_total_train - train_1 - train_2;
    unsigned int valid_3 = n_total_valid - valid_1 - valid_2;
    unsigned int test_3 = n_total_test - test_1 - test_2;

    file_info.open(path_info_valid +"/experimento_4.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 4\n";
    file_info << "Correspondencia ficheros valid con los ficheros originales: \n\n";
    file_info.close();
    file_info.open(path_info_test +"/experimento_4.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 4\n";
    file_info << "Correspondencia ficheros test con los ficheros originales: \n\n";
    file_info.close();

    random_set(&n_total_train, &n_total_valid,&n_total_test, path_depth_map_4,path_label_4,path_train,path_valid, path_test,path_train_txt, path_valid_txt, path_test_txt,(path_info_valid +"/experimento_4.txt"),(path_info_test +"/experimento_4.txt"));
    unsigned int train_4 = n_total_train - train_1 - train_2 - train_3;
    unsigned int valid_4 = n_total_valid - valid_1 - valid_2 - valid_3;
    unsigned int test_4 = n_total_test - test_1 - test_2 - test_3;

    file_info.open(path_info_valid +"/experimento_5.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 5\n";
    file_info << "Correspondencia ficheros valid con los ficheros originales: \n\n";
    file_info.close();
    file_info.open(path_info_test +"/experimento_5.txt",std::fstream::out | std::fstream::app);
    file_info << "Experimento 5\n";
    file_info << "Correspondencia ficheros test con los ficheros originales: \n\n";
    file_info.close();

    random_set(&n_total_train, &n_total_valid,&n_total_test, path_depth_map_5,path_label_5,path_train,path_valid, path_test,path_train_txt, path_valid_txt, path_test_txt,(path_info_valid +"/experimento_5.txt"),(path_info_test +"/experimento_5.txt"));
    unsigned int train_5 = n_total_train - train_1 - train_2 - train_3 - train_4;
    unsigned int valid_5 = n_total_valid - valid_1 - valid_2 - valid_3 - valid_4;
