...
int main (int argc, char ** argv){
    ...
    std::string directorio_pcd ="/home/alejandro/catkin_ws/src/mis_programas/bagfiles/experimento2_64_real/point_clouds/37.880000000.pcd";
    ...
    try{
        bag.open("/home/alejandro/catkin_ws/src/mis_programas/bagfiles/experimento2_64_real/experimento2_64_real.bag",rosbag::BagMode::Read);
    }
    ...
    for(rosbag::ConnectionInfo const *ci: topic_list_view.getConnections() ){
        ...
        while(odom1_view_it != odom1_view.end()){
            ...
            if(odom1_t.header.stamp.toSec() == 37.880000000 ){
        ...
        while(odom2_view_it != odom2_view.end()){
            ...
            if(odom2_t.header.stamp.toSec() == 37.880000000 ){
  ...
  std::cout << "Loaded "
    ...
    std::ofstream out("prueba_paralelepipedo.txt");
  ...
  for (size_t i = 0; i < cloud->points.size (); ++i){
    /*std::cerr << "X:  " << cloud->points[i].x
        ...
        if ( ((_x > (x_pose - 0.325)) && (_x < (x_pose + 0.325))) && ((_y > (y_pose-0.325))&&(_y < (y_pose + 0.325))) && ((_z > (z_pose - 0.1)) && (_z < (z_pose + 0.1))) ){
    ...
    std::cout << "x_pose: " << x_pose << "\n";
    std::cout << "y_pose: " << y_pose << "\n";
    std::cout << "z_pose: " << z_pose << "\n";
    std::cout << "r_pose: " << r_pose << "\n";

    cv::namedWindow("BW_Image", CV_WINDOW_AUTOSIZE);
