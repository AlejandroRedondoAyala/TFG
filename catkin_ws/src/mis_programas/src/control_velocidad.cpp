...
void odom_callback(const nav_msgs::Odometry::ConstPtr & msg){
    ...
    Eigen::Vector3d desired_position (max_vel*10*dt , 0.0, 0.0); //Poner max_vel*10*dt, en el eje en el que se quiera controlar linealmente la velocidad
    ...
    //trajectory_msg.points[0].transforms[0].translation.x = 0.0; 
...
int main (int argc, char** argv){
    ...
    trajectory_pub = n.advertise<trajectory_msgs::MultiDOFJointTrajectory>("/firefly/command/trajectory", 10);

    ros::Subscriber odom_sub = n.subscribe("/firefly/odometry_sensor1/odometry", 10, odom_callback);
